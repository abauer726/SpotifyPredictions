...
do.chunk.bart <- function(chunkid, folddef, Xdat, Ydat, ...){
  ...
  data.frame(fold = chunkid,
    train.error = mean((Ytrain - predYtr$yhat.train.mean)^2), # Training error for each fold
    val.error = mean((Ytest - predYval$yhat.test.mean)^2))  # Validation error for each fold
}
```
...
for (i in seq(5)){
  tmp = do.chunk.bart(chunkid=i, folddef=folds, Xdat=Xtrain, Ydat=Ytrain)
...
```{r}
pred.YTest = gbart(Xtrain, Ytrain, x.test = Xtest)
# Test mse
test_mse = mean((Ytest - pred.YTest$yhat.test.mean)^2)
test_mse
```
