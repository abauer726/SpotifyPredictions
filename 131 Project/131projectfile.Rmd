...
```{r}
# from page 40 plot fitted vs actual model
##build BART regre


# generate plot
plot_y_vs_yhat(bart_machine_cv)
```
predict:
```{r}
predict(bart_machine_cv, test, type = "prob", prob_rule_class = NULL, verbose = TRUE)
```
...
```{r}
# plot prediction - grant added this code but it doesn't work
plot(apply(pred, 2, mean), f(Xtestbart))
```
