...
Next, we decided to have 100 neighbors considered, so we looped through all 100 of the neighbors for each different chunk id.
...
As shown above, we found that a 98-NN classifier would be best. Finally, we were able to calculate the test MSE using the knn prediction. 
...
Thus, we found or training MSE to be 82.51655 and our test MSE to be 83.92452 for this model. This is a low MSE, and the training and test MSEs being so similar led us to believe that the k-NN model properly prevented overfitting. 
...
Based on these results, the best model of the four was MARS. 
...
plot(test$popularity, mars.pred, xlab="y", ylab=expression(hat(y)))
```
