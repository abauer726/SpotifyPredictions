...
Many of the current hits are actually created from machine learning algorithms. Whether they are enjoyable to listen to is another topic, but nevertheless these machine-created hits successfully used algorithms for lyrics, beats, timing, and other factors to make these song popular. We were inspired by this topic in our research question, especially when wondering which factors in these algorithms had the most weight in the song creation. 
...
We decided to have 100 neighbors considered, so we looped through all 100 of the neighbors for each different chunk id.
...
Ytrainknn <- train$popularity # from lab 5
...
As shown above, we found that a 98-NN classifier would be best. Finally, we were able to calculate the test MSE using the k-NN prediction. 
...
# head(pred.Ytrainknn)
# get training MSE
mean((pred.Ytrainknn$pred - Ytrainknn)^2) # 82.51655
...
head((pred.Ytestknn$pred - Ytestknn)^2) # 174.61735  58.72564 204.08163  30.92722  56.25000  37.35954
mean((pred.Ytestknn$pred - Ytestknn)^2) # 83.92452
```

Thus, we found or training MSE to be 82.51655 and our test MSE to be 83.92452 for this model. We found this to be a relatively low MSE, and the training and test MSEs being so similar led us to believe that the k-NN model properly prevented overfitting. 
...
# BART Implementation
Ytrainbart <- train$popularity # from lab 5
...
A test MSE of 407.7647 was very surprising and almost immediately disqualified BART as a contender. We weren't too worried about overfitting because the algorithms we chose accounted for that, and we didn't have too many predictors in comparison to the number of observations. In addition, our train and test errors were fairly similar.
...
Based on these results, the best performing model of the four was MARS. 

### Performance of our Prediction


reproducing the plot from above, comment on tuning - # terms and degree impact on rmse

```{r, message = FALSE, warning = FALSE}
plot(cv_mars)
```
variable importance plot - i think we should get rid of this since it is so weird and gives away the fact that avg_art_pop has way too much influence

```{r, message = FALSE, warning = FALSE}
vip(cv_mars)
```

A good way to determine how well the model predictions performed in comparison to the test data is a plot of the predictions against the true values. In the plot, if the values were perfectly predicted, the points would fall along the y=x line, for the $\hat{y}$s would match the $y$s. In this case, our MARS prediction is decent.

grant's plot - feel free to change color, black line is linear trend line
...
take a sample of the residuals for visual purposes: we want to be able to see the data in smaller quantity

```{r}
resid <- test$popularity - mars.pred # get residuals
df_mars_visual <- data.frame(test$popularity, mars.pred, resid) # data frame of actual, predicted, and resoduals 
sample_mars <- df_mars_visual[sample(nrow(df_mars_visual), 300), ] # 300 value sample of the datframe
```

same scatterplot as above but with sample of data (for visualization purposes)
...
```
residual plot: actual vs predicted with residual line
...
NOTES FROM ANNA:
For prediction code, make sure to comment on how the MSE of 74.13, indicating that on average, the prediction code was 8.6 percent off of the actual song popularity (that would be RMSE).
- hard to determine what accuracy is considered "good"
