...
For the next step of our exploratory data analyis, we want to take an even closer look at relationships between our predictors and response through a correlation matrix and subsequent heatmap. First, in order to create our correlation matrix and heatmap, we code our dummy variables into as.factor variables:
...
After we explored our data visually, we began to build our models. We looked at the characteristics of our data, such as the number of observations, type of data (qualitative vs quantitative), etc., and did some outside research on models that might fist best with our data. We found that an elastic net model, a Bayesian additive regression trees (BART) model, a multivariate adaptive regression splines (MARS) model, and a k-nearest neighbors (KNN) model would suit our regression-based research question and dataset well. Below, we explore the reasons why we chose each model, we fit each of these models to our training data, and then we calculate the test MSE of each model to compare later.
...
Thus, we found or training MSE to be 82.51655 and our test MSE to be 83.92452 for this model. We found this to be a relatively low test MSE, and the training and test MSEs being so similar led us to believe that the k-NN model properly prevented overfitting. 
...
With MARS being our best-performing model of the four we chose to explore, we want to evaluate how well it did in predicting on our test data. Below, we reproduce a plot we had shown above of the test RMSE of our tuned MARS model by terms and degree.

```{r, message = FALSE, warning = FALSE}
plot(cv_mars)
```

With the MARS model having a test MSE of 74.13 and a RMSE of 8.6 (as displayed in the plot above), this indicates that on average, our MARS prediction of song popularity was 8.6 percentiles off off the actual song popularity for each of the ~30,000 test observation. 

To determine how well the model predictions performed in comparison to the test data, we can also take a look at a plot of the predictions against the true values. In the plot below, if the values were perfectly predicted, the points would fall along the y=x line, for the $\hat{y}$s would match the $y$s. 
...
We can take an even closer look at this relationship between actual values and predicted values by taking a sample of the data for visual purposes, as we want
to be able to see the data in smaller quantity. This sample is taken below:
...
By plotting a sample of the data, we can see much clearer this relationship between actual and predicted values, and we see that they fall relatively closely to the y=x line. 
