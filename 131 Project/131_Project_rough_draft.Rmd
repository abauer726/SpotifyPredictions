...
There are several things that we extrapolated from the panel of plots: mainly, many of the predictors, such as `valence`, `acousticness`, `danceability`, `duration_ms`, and `tempo` all have fairly weak correlations with `popularity`. One can also see that many songs in the dataset appear to be around 0 for `speechiness`, `liveness`, and `acousticness`. Regarding the categorical variables `explicit` and `mode`, there is a clear skew towards clean songs and songs in a major key.
...
Finally, an analysis of the distribution of the response `popularity` allowed us to see that it was fairly normally-distributed.
...
The k-nearest neighbors regression algorithm is a non-parametric method that takes observations from different neighbors to approximate the association between the predictors and the outcome (in our case song popularity). k-NN was a good fit for our research question because the function can have many function forms other than linear, which is what we were looking for. 
...
numneighbor
```
Thus, we found that a 50-NN classifier would be best. Finally, we were able to calculate the test MSE using the knn function. 
...
This model was a good choice for our data because it's a regression model, which fits our mostly quantitative variables, and it improves on the lasso and ridge techniques. Additionally, the lasso and ridge methods account for collinearity, so that eliminated our need to investigate collinearity between predictors. 
```{r}
# load packages
...
Once we had our elastic net model, we wanted to make a prediction on our test data. To do this, we used the predict() function to get the test mean square error. Later we compared the test MSEs of different functions to decide which model is optimal for our data (in the 'Comparing Models' section). 
```{r}
