...
summary(algae)
```
From the summary table above, we can see the number of missing values and the mean of each chemical. The below code chunk calculates the variance whole accounting for the missing values in each of the chemical columns.
```{r, echo = TRUE}
var(x = algae, na.rm=TRUE)
```
